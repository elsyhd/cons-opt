...
% Default command-line runs the NSGA-II study.
...
if isfield(config, 'writeExcel') && config.writeExcel
    writeMetricsExcel(metrics);
end

printConstellationSummary(metrics);

if config.enablePlots
    plotTargetMetrics(metrics);
end
...
function writeMetricsExcel(metrics)

excelFile = 'constellation_metrics.xlsx';
...
summaryTable = table( ...
    ...
    'VariableNames', { ...
        ...
        'GlobalMeanVisibleSatellites', ...
        'GlobalMeanGDOP', ...
        'GlobalMeanGDOPDeviation', ...
        'Cost_USD'});

targetTable = table( ...
    string(metrics.siteNames(:)), ...
    metrics.availabilityByTarget, ...
    metrics.meanVisibleAll, ...
    metrics.stdVisibleAll, ...
    metrics.maxVisiblePerTarget, ...
    metrics.meanGDOPAllVisible, ...
    metrics.stdGDOPAllVisible, ...
    'VariableNames', { ...
        'Site', ...
        'Availability', ...
        'MeanVisibleSatellites', ...
        'StdVisibleSatellites', ...
...
timeSeriesTable = table( ...
    metrics.tspan(:), ...
    hours(metrics.tspan(:) - metrics.tspan(1)), ...
    metrics.globalMeanVisibleOverTime(:), ...
    metrics.globalMeanGDOPOverTime(:), ...
    metrics.globalAvailabilityOverTime(:), ...
    'VariableNames', { ...
        'TimeUTC', ...
        'ElapsedHours', ...
        'GlobalMeanVisibleSatellites', ...
...
writetable(targetTable, excelFile, 'Sheet', 'Targets');
writetable(timeSeriesTable, excelFile, 'Sheet', 'GlobalTimeSeries');

fprintf('Excel metrics saved to %s\n', excelFile);
